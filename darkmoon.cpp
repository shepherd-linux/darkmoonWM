 ...
 *
 * The event handlers of darkmoon are organized in an array which is accessed
...
void applyrules(Client *c)
{
	...
	{
		r = &rules[i];
		if ((!r->title || strstr(c->name, r->title)) && (!r->_class || strstr(_class, r->_class)) && (!r->instance || strstr(instance, r->instance)))
		{
...
createmon(void)
{
	...
	m = new Monitor; //ecalloc(1, sizeof(Monitor));
...
void manage(Window w, XWindowAttributes *wa)
{
	...
	c = new Client; //ecalloc(1, sizeof(Client));
	c->win = w;
	/* geometry */
	c->x = c->oldx = wa->x;
	c->y = c->oldy = wa->y;
...
void setup(void)
{
	...
	/* init appearance */
	scheme = new Clr *; //ecalloc(LENGTH(colors), sizeof(Clr *));
	for (i = 0; i < LENGTH(colors); i++)
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
					PropModeReplace, (unsigned char *)"darkmoon", 3);
...
void spawn(const Arg *arg)
{
	...
	if (fork() == 0)
	{
		...
		fprintf(stderr, "darkmoon: execvp %s", ((char **)arg->v)[0]);
		perror(" failed");
		exit(EXIT_SUCCESS);
	}
}
...
void updatebars(void)
{
	...
	XClassHint ch = {"darkmoon", "darkmoon"};
	for (m = mons; m; m = m->next)
	{
		if (m->barwin)
			continue;
...
int xerror(Display *dpy, XErrorEvent *ee)
{
	if (ee->error_code == BadWindow || (ee->request_code == X_SetInputFocus && ee->error_code == BadMatch) || (ee->request_code == X_PolyText8 && ee->error_code == BadDrawable) || (ee->request_code == X_PolyFillRectangle && ee->error_code == BadDrawable) || (ee->request_code == X_PolySegment && ee->error_code == BadDrawable) || (ee->request_code == X_ConfigureWindow && ee->error_code == BadMatch) || (ee->request_code == X_GrabButton && ee->error_code == BadAccess) || (ee->request_code == X_GrabKey && ee->error_code == BadAccess) || (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "darkmoon: fatal error: request code=%d, error code=%d\n",
...
{
	die("darkmoon: another window manager is already running");
	return -1;
}
...
int main(int argc, char *argv[])
{
	if (argc == 2 && !strcmp("-v", argv[1]))
		die("darkmoon-VERSION");
	else if (argc != 1)
		die("usage: darkmoon [-v]");
	if (!setlocale(LC_CTYPE, "") || !XSupportsLocale())
		fputs("warning: no locale support\n", stderr);
	if (!(dpy = XOpenDisplay(NULL)))
		die("darkmoon: cannot open display");
	checkotherwm();
	setup();
#ifdef __OpenBSD__
